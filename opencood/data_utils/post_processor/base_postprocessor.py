...
class BasePostprocessor(object):
    ...
    def generate_object_center_tumv2x(self,
                               lidar_anno_frame):
        """
        Retrieve all objects in a format of (n, 7), where 7 represents
        x, y, z, l, w, h, yaw or x, y, z, h, w, l, yaw.
        ...
        """
        output_dict = {}
        lidar_range = self.params['anchor_args']['cav_lidar_range']
        ...
        for id in lidar_anno_frame['objects']:
            ...
            dim = np.asarray(object_data['cuboid']['val'][7:], dtype=np.float32) # lwh 
            dim_hwl = dim[[2, 1, 0]]
            ...
            gt_box = np.concatenate([loc, dim_hwl, -yaw], axis=None).reshape(1, 7)

            bbx_lidar = box_utils.mask_boxes_outside_range_numpy(gt_box, lidar_range_z_larger, 'hwl')

            if bbx_lidar.shape[0] > 0:
                output_dict.update({id: bbx_lidar})    

        object_np = np.zeros((self.params['max_num'], 7))
        mask = np.zeros(self.params['max_num'], dtype=bool)
        object_ids = []
        for i, (object_id, object_bbx) in enumerate(output_dict.items()):
            object_np[i] = object_bbx[0, :]
            mask[i] = True
