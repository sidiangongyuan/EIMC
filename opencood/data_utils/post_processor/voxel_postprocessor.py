...
class VoxelPostprocessor(BasePostprocessor):
    ...
    def post_process_detr(self,data_dict,output_dict):
        # TODO: we will do points protjec into image for visualization.
        pred_box3d_list = []
        ...
        # only for vis
        # points = pred_box3d_tensor.unsqueeze(-1).repeat(4,1,1,1,1) # 4, M, 8, 3, 1   # 4 is the number of view 
        # ones = torch.ones_like(points[..., :1, :]) # 4, M, 8, 4, 1
        ...
        # points_2d = torch.matmul(intrins, points_cam).squeeze()  # 4, M, 8, 3
        # # points_2d = []
        # #
        ...
        # images =  cav_content['original_imgs'][0][0]
        # # visualize point
        # imgs = []
        # points_ =[]
        # for i in range(4):
        #     imgs.append(images[i].permute(1,2,0))
        #     points_.append(points_2d[i])
        # self.visualize_projected_points(imgs,points_)

        # remove large bbx 
        # TODO: if there anything about large car ? 
        keep_index_1 = box_utils.remove_large_pred_bbx(pred_box3d_tensor)
        keep_index_2 = box_utils.remove_bbx_abnormal_z(pred_box3d_tensor)
        keep_index = torch.logical_and(keep_index_1, keep_index_2)
        pred_box3d_tensor = pred_box3d_tensor[keep_index]
        pred_score_tensor = pred_score_tensor[keep_index]
