...
from opencood.data_utils.datasets.basedataset.tumv2x_basedataset import TUMV2XBaseDataset
from opencood.visualization.simple_plot3d import canvas_bev



class TUMV2XIntermediateFusionDataset(TUMV2XBaseDataset):
    ...
    ########################## vis lidar ##########################
    def vis_LiDARs(self, lidar_nps, bbox=None):
        pc_range = self.params['preprocess']['cav_lidar_range']
        plt.figure(figsize=[(pc_range[3]-pc_range[0])/10, (pc_range[4]-pc_range[1])/10])
        pc_range = [int(i) for i in pc_range]
        canvas = canvas_bev.Canvas_BEV_heading_right(canvas_shape=((pc_range[4]-pc_range[1])*5, (pc_range[3]-pc_range[0])*5),
        ...
        for lidar_np, color in zip(lidar_nps, colors):
        ...
        if bbox is not None:
            corners_bbox = box_utils.boxes_to_corners_3d(bbox,'hwl')
            canvas.draw_boxes(corners_bbox,colors=(0,255,0))
        plt.axis("off")
        plt.imshow(canvas.canvas)
        plt.tight_layout()
        plt.savefig('/mnt/sdb/public/data/yangk/result/heal/vis/example1', transparent=False, dpi=200)
        plt.clf()
        plt.close()
    ########################## vis lidar ##########################

     
    def get_item_single_car(self, selected_cav_base, ego_cav_base, transformation_matrix=None):
        """
        ...
        if self.load_camera_file:
            ...
            for idx, img in enumerate(camera_data_list): # s1,s2,north
                # camera_to_lidar, camera_intrinsic = self.get_ext_int(params, idx)
                camera_intrinsic = params[list_params[idx]]['intrinsic']
                lidar_to_image = params[list_params[idx]]['lidar_to_image']
                camera_to_lidar = params[list_params[idx]]['camera_to_lidar']
        ...
        selected_cav_processed.update({"lidar_np": projected_lidar})
    ...
    def __getitem__(self, idx):
        ...
        # TODO: hard code
        # pairwise_t_matrix = ego_cav_base['params']['pairwise_t_matrix']
        pairwise_t_matrix = np.tile(np.eye(4), (2, 2, 1, 1)) # (L, L, 4, 4)
        pairwise_t_matrix[1,0] = ego_cav_base['params']['pairwise_t_matrix']
        pairwise_t_matrix[0,1] = np.linalg.inv(ego_cav_base['params']['pairwise_t_matrix'])

        
        
        # merge preprocessed features from different cavs into the same dict
        cav_num = len(cav_id_list)
        object_stack = ego_cav_base['params']['labels']['object_np']
        obj_mask = ego_cav_base['params']['labels']['mask']
        filtered_objects = object_stack[obj_mask]
        ...
        for _i, cav_id in enumerate(cav_id_list):
            ...
            selected_cav_processed = self.get_item_single_car(
                selected_cav_base,
                ego_cav_base, pairwise_t_matrix[_i,0])
            ...
            lidar_nps.append(selected_cav_processed['lidar_np'])
        ...
        # self.vis_LiDARs(lidar_nps, object_stack)
        # make sure bounding boxes across all frames have the same number
        object_bbx_center = \
            np.zeros((self.params['postprocess']['max_num'], 7))
        mask = np.zeros(self.params['postprocess']['max_num'])
        object_bbx_center[:filtered_objects.shape[0], :] = filtered_objects
        mask[:filtered_objects.shape[0]] = 1
        ...
        if self.load_camera_file:
            merged_image_inputs_dict = merge_features_to_dict(agents_image_inputs, merge='cat')
