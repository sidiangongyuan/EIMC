...
from opencood.tools.test_time import BlockTimeProfiler
torch.multiprocessing.set_sharing_strategy('file_system')

def test_parser():
    parser = argparse.ArgumentParser(description="synthetic data generation")
    parser.add_argument('--model_dir', type=str,
                        help='Continued training path', default='/mnt/sdb/public/data/yangk/result/heal/dair-v2x_attn_2025_01_05_11_42_28/')
...
def main():
    print("This is the script for test inference fps and more ap metric.")
    set_seed(0)
    opt = test_parser()
    ...
    if 'heter' in hypes:
        ...
        x_min, x_max = hypes['cav_lidar_range'][0], hypes['cav_lidar_range'][3]
        y_min, y_max = hypes['cav_lidar_range'][1], hypes['cav_lidar_range'][4]
    ...
    result_stat = {
        ...
        0.75:{'tp': [], 'fp': [], 'gt': 0, 'score': []},
        0.8: {'tp': [], 'fp': [], 'gt': 0, 'score': []},
                0.85: {'tp': [], 'fp': [], 'gt': 0, 'score': []},
                0.9: {'tp': [], 'fp': [], 'gt': 0, 'score': []},
                0.95: {'tp': [], 'fp': [], 'gt': 0, 'score': []}
                }
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            tot_times = tot_times + (time.time()-start_time)
    ...
    fps = len(data_loader) / tot_times
    print(f"FPS: {fps:.2f}")
    file_path = os.path.join(opt.model_dir, 'fps.txt')
    with open(file_path, "a") as file:
        file.write(f"FPS: {fps:.2f}\n")

    _, ap50, ap70, ap80, ap85, ap90, ap95 = eval_utils.eval_final_results_more_metric(result_stat,
                                opt.model_dir)
