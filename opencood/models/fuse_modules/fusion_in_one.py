...
class AttFusion(nn.Module):
    ...
    def forward(self, xx, record_len, affine_matrix=None):
        _, C, H, W = xx.shape
        B, L = affine_matrix.shape[:2]
        ...
        for b in range(B):
            ...
            cav_num = x.shape[0]
            x = x.reshape(cav_num, C, -1).permute(2, 0, 1) #  (H*W, cav_num, C), perform self attention on each pixel.
            h = self.att(x, x, x)
            h = h.permute(1, 2, 0).reshape(cav_num, C, H, W)[0, ...]  # C, W, H before
            out.append(h)
