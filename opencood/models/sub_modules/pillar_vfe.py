    ...
    def forward(self, batch_dict):
        ...
        Args:
            batch_dict: Dictionary containing:
                ...
                - voxel_num_points: [M,]
                - voxel_coords: [M, 4]
                - (optional) occ_voxel_features: [N, 32, 4]
                - (optional) occ_voxel_num_points: [N,]
                - (optional) occ_voxel_coords: [N, 4]
        
        Returns:
            batch_dict with:
                - pillar_features: [M, 64], after PFN
                - (optional) occ_features: [N, 64], after PFN
        """
        # Process voxel features
        def process_voxel_features(voxel_features, voxel_num_points, coords):
        ...
        # Process primary voxel features
        voxel_features = batch_dict['voxel_features']
        voxel_num_points = batch_dict['voxel_num_points']
        coords = batch_dict['voxel_coords']
        batch_dict['pillar_features'] = process_voxel_features(voxel_features, voxel_num_points, coords)

        # Check and process occ_voxel_features if present
        if 'occ_voxel_features' in batch_dict:
            occ_voxel_features = batch_dict['occ_voxel_features']
            occ_voxel_num_points = batch_dict['occ_voxel_num_points']
            occ_coords = batch_dict['occ_voxel_coords']
            batch_dict['occ_features'] = process_voxel_features(occ_voxel_features, occ_voxel_num_points, occ_coords)
