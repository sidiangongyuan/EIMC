...
def warp_affine_simple_3d(src, M, dsize,
                          mode='bilinear',
                          padding_mode='zeros',
                          align_corners=False):
    """
    Perform 3D affine transformation on a source volume.

    Args:
        src: torch.tensor
            Source volume of shape [B, C, D, H, W].
        M: torch.tensor
            Affine transformation matrix of shape [B, 3, 4].
        dsize: tuple
            Target size as (D, H, W).
        mode: str
            Interpolation mode to calculate output values ('bilinear' or 'nearest').
        padding_mode: str
            Padding mode for outside grid values ('zeros', 'border', or 'reflection').
        align_corners: bool
            Geometrically, whether to align the corners of input and output grids.

    Returns:
        torch.tensor
            Transformed volume of shape [B, C, D, H, W].
    """
    B, C, D, H, W = src.size()

    # Generate the 3D affine grid
    grid = F.affine_grid(M, [B, C, dsize[0], dsize[1], dsize[2]], align_corners=align_corners).to(src)

    # Sample the source volume with the generated grid
