...
def normalize_pairwise_tfm_3d(pairwise_t_matrix, D, H, W, discrete_ratio, downsample_rate=1):
    """
    Normalize the pairwise transformation matrix to affine matrix needed by torch.nn.functional.affine_grid() for 3D.
    
    Args:
        pairwise_t_matrix: torch.tensor
            [B, L, L, 4, 4], B batch size, L max_cav
        D: num
            Feature map depth
        H: num
            Feature map height
        W: num
            Feature map width
        discrete_ratio * downsample_rate: num
            One voxel on the feature map corresponds to the actual physical distance

    Returns:
        affine_matrix: torch.tensor
            [B, L, L, 3, 4]
    """
    # Extract the 3D affine transformation components [3x4] from the [4x4] transformation matrix
    affine_matrix = pairwise_t_matrix[:, :, :, :3, :][:, :, :, :, [0, 1, 2, 3]]  # [B, L, L, 3, 4]
    
    # Scale the x, y, and z axes to account for the different dimensions
    affine_matrix[..., 0, 1] = affine_matrix[..., 0, 1] * H / W  # Scale y-axis in terms of width
    affine_matrix[..., 0, 2] = affine_matrix[..., 0, 2] * D / W  # Scale z-axis in terms of width
    affine_matrix[..., 1, 0] = affine_matrix[..., 1, 0] * W / H  # Scale x-axis in terms of height
    affine_matrix[..., 1, 2] = affine_matrix[..., 1, 2] * D / H  # Scale z-axis in terms of height
    affine_matrix[..., 2, 0] = affine_matrix[..., 2, 0] * W / D  # Scale x-axis in terms of depth
    affine_matrix[..., 2, 1] = affine_matrix[..., 2, 1] * H / D  # Scale y-axis in terms of depth
    
    # Normalize the translation components (last column)
    affine_matrix[..., 0, 3] = affine_matrix[..., 0, 3] / (downsample_rate * discrete_ratio * W) * 2
    affine_matrix[..., 1, 3] = affine_matrix[..., 1, 3] / (downsample_rate * discrete_ratio * H) * 2
    affine_matrix[..., 2, 3] = affine_matrix[..., 2, 3] / (downsample_rate * discrete_ratio * D) * 2
